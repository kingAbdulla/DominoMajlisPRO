...
public sealed class StoreCatalogLivingVisualManifestProvider : ILivingVisualManifestProvider
{
    ...
    private async Task<LivingVisualAssetManifest?> ToManifestAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var packageRoot = string.IsNullOrWhiteSpace(asset.LivingPackagePath) ||
            IsDirectModelPath(asset.LivingPackagePath)
    ...
    private async Task<LivingVisualAssetManifest?> CreateDefaultPackageManifestAsync(
        CancellationToken cancellationToken,
        CatalogAssetDisplay? asset = null)
    {
        ...
        return import.Package == null
            ? null
            : ToManifest(import.Package, asset);
    }

    private static LivingVisualAssetManifest ToManifest(
        LivingEmblemPackage package,
        CatalogAssetDisplay? asset = null) => new()
    {
        PackageId = string.IsNullOrWhiteSpace(asset?.LivingPackageId)
            ? package.Manifest.PackageId
            : asset.LivingPackageId,
        PackageManifestPath = string.IsNullOrWhiteSpace(asset?.LivingPackageManifestPath)
            ? package.ManifestPath
            : asset.LivingPackageManifestPath,
        ...
        Version = string.IsNullOrWhiteSpace(asset?.LivingPackageVersion)
            ? package.Manifest.Version
            : asset.LivingPackageVersion,
        IsPublished = true,
