...
{
    static string FilePath => Path.Combine(FileSystem.AppDataDirectory, "players.json");
    static string ImagesFolder => Path.Combine(FileSystem.AppDataDirectory, "player_images");
    ...
    public static async Task<List<PlayerProfileModel>> LoadPlayersAsync()
    {
        try
        {
            ...
            var players = JsonSerializer.Deserialize<List<PlayerProfileModel>>(json) ?? new();
    ...
    public static async Task<PlayerProfileModel?> GetPlayerByNameAsync(string playerName)
    {
        ...
        string normalizedName = PlayerIdentityService.NormalizePlayerName(playerName);
        return players.FirstOrDefault(x => PlayerIdentityService.NormalizePlayerName(x.PlayerName) == normalizedName);
    }

    public static async Task UpdatePlayerStatsAsync(string playerName, bool wonMatch)
    {
        if (string.IsNullOrWhiteSpace(playerName))
            return;
        var players = await LoadPlayersAsync();
        ...
        {
            string normalizedName = PlayerIdentityService.NormalizePlayerName(playerName);
            player = players.FirstOrDefault(x => PlayerIdentityService.NormalizePlayerName(x.PlayerName) == normalizedName);
        }
        if (player == null)
            return;
    ...
    {
        var players = await LoadPlayersAsync();
        int index = players.FindIndex(x => x.PlayerId == updatedPlayer.PlayerId);
        if (index < 0)
    ...
    public static async Task SetBuiltInAvatarAsync(string playerId, string avatarImage)
    {
        var players = await LoadPlayersAsync();
        var player = players.FirstOrDefault(x => x.PlayerId == playerId);
        if (player == null)
            throw new Exception("لم يتم العثور على اللاعب.");
        player.AvatarImage = string.IsNullOrWhiteSpace(avatarImage) ? "player_card.png" : avatarImage;
        ...
        PlayerEngine.Normalize(player);
        PlayerTimelineService.AddEvent(player, "تغيير الصورة الشخصية", "تم اعتماد Avatar جديد", "🖼", "#D4AF37");
        await SavePlayersAsync(players);
    }

    public static async Task SetProfileImageFromDeviceAsync(string playerId, FileResult file)
    {
        if (file == null)
        ...
        if (string.IsNullOrWhiteSpace(extension))
            extension = ".jpg";
        string targetPath = Path.Combine(ImagesFolder, $"{playerId}_{DateTime.Now:yyyyMMdd_HHmmss}{extension}");
        ...
        player.LastUpdatedAt = DateTime.Now;
        PlayerEngine.Normalize(player);
        PlayerTimelineService.AddEvent(player, "تغيير الصورة الشخصية", "تم اختيار صورة من الجهاز", "🖼", "#D4AF37");
        await SavePlayersAsync(players);
    }
    ...
    {
        var storeAvatar = ResolveEquippedStoreAvatar(player.PlayerId);
        return storeAvatar ?? PlayerEngine.GetImageSource(player);
    }

    static ImageSource? ResolveEquippedStoreAvatar(string playerId)
    {
        ...
        try
        {
            ...
            {
                var identity = await PlayerVisualIdentityResolver.ResolveAsync(playerId);
                return InventoryDisplayResolver.ResolveOptionalImageSource(identity.Avatar?.PreviewImage);
