...
public static class PlayerAssetInventoryService
{
    ...
    public static async Task<IReadOnlyList<PlayerOwnedStoreItem>>
        GetInventoryForPlayerAsync(string playerId)
    {
        ...
        var purchased = loaded
            .Where(IsValidOwnedPlayerAsset);
        return CreateDefaultAssets(playerId)
            ...
            .Select(group => group
                ...
                .ThenByDescending(item => !Same(item.Source, "Default"))
                .First())
            .ToList();
    }
    ...
    private static bool IsValidOwnedPlayerAsset(PlayerOwnedStoreItem? item)
    {
        ...
        return PlayerTypes.Contains(
            StoreAssetCatalogService.CanonicalTypeId(item.StoreTypeId));
    }

    public static Task<bool> AddPurchasedAssetAsync(
        string playerId,
        string assetId,
        string assetType,
        string source = "StorePurchase",
        ...
        string? collectionId = null)
    {
        ...
        return PlayerInventoryService.AddOwnedItemAsync(
            playerId, assetId, assetType, source,
            seasonId: seasonId, collectionId: collectionId);
    }
        ...
        string assetType)
    {
        ...
        if (Same(assetType, StoreProductAssetType.Avatar.ToString()) &&
            defaultAvatar != null)
        {
            await PlayerProfileService.SetBuiltInAvatarAsync(
                playerId, defaultAvatar.Image);
    ...
    public static async Task<PlayerOwnedStoreItem?> GetEquippedAsync(
        string playerId,
        string assetType)
    {
        ValidatePlayerType(assetType);
        return (await GetInventoryForPlayerAsync(playerId))
            .FirstOrDefault(item =>
                item.IsEquipped &&
                Same(StoreAssetCatalogService.CanonicalTypeId(item.StoreTypeId),
                    assetType));
    }

    private static IEnumerable<PlayerOwnedStoreItem> CreateDefaultAssets(
        string playerId)
    {
        var acquiredAt = new DateTime(
    ...
    {
        if (!PlayerTypes.Contains(
                StoreAssetCatalogService.CanonicalTypeId(assetType)))
            throw new ArgumentException(
                "AssetType is not a player cosmetic.", nameof(assetType));
    }

    private static bool Same(string? left, string? right) =>
        string.Equals(left?.Trim(), right?.Trim(),
