...
public sealed class TypographyIdentityPreset
{
    ...
    private static string NormalizeToken(string? value, string fallback) => string.IsNullOrWhiteSpace(value) ? fallback : value.Trim();
    ...
    private static string ValidColor(string? value, string fallback)
    {
        ...
        if (string.IsNullOrWhiteSpace(token) || token[0] != '#')
            return fallback;
        var hex = token[1..];
        return (hex.Length == 6 || hex.Length == 8) && hex.All(Uri.IsHexDigit) ? token : fallback;
    }
}

public sealed record TypographyFontOption(string Family, string DisplayName, bool ArabicFriendly);
...
public static class TypographyFontCatalog
{
    ...
    public static IReadOnlyList<string> FontFamilies { get; } = Fonts.Select(font => font.Family).ToArray();

    public static string ResolveFamily(string? family)
    {
        var token = family?.Trim();
        return Fonts.Any(font => string.Equals(font.Family, token, StringComparison.Ordinal)) ? token! : DefaultFontFamily;
    }
}
